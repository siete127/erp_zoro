...
def create_lote(payload: dict[str, Any], current_user: dict[str, Any]) -> dict[str, Any]:
    ...
    if not numero_lote:
        raise HTTPException(status_code=400, detail="NumeroLote requerido")

    if cantidad < 0:
        raise HTTPException(status_code=400, detail="CantidadInicial no puede ser negativa")
        ...
        if cantidad > 0:
            conn.execute(
                ...
                """),
                {
                    ...
                    "lote_id": lote_id,
                    "cantidad": cantidad,
                    "referencia": numero_lote,
                    "notas": "Entrada inicial por creación de lote",
                },
            )
...
def consumir_lote(
    ...
    if cantidad > disponible:
        raise HTTPException(
            status_code=400,
            detail=f"Cantidad solicitada ({cantidad}) supera disponible ({disponible})",
        )
    ...
    with get_transaction() as conn:
        conn.execute(
            text("""
                UPDATE ERP_LOTES
                SET CantidadActual = CantidadActual - :qty
                WHERE Lote_Id = :id
            """),
            {
                "qty": cantidad,
        ...
        conn.execute(
            ...
            """),
            {
                ...
                "notas": payload.get("Notas") or "Salida/consumo de lote",
            },
        )

    return {
        "success": True,
        "message": f"Consumidas {cantidad} unidades del lote",
        "data": {
