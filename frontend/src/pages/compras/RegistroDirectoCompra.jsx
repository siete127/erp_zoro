...
/**
 * Registro directo de compra con factura (sin flujo de autorización).
 * Aplica para compras espontáneas que ya cuentan con factura del proveedor.
 */
...
export default function RegistroDirectoCompra() {
  ...
  useEffect(() => {
    ...
    load().catch(() => notify('Error cargando catálogos', 'error'));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.FacturaReferencia) { notify('Ingresa el número de factura del proveedor', 'error'); return; }
    if (items.some(it => !it.Descripcion || Number(it.Cantidad) <= 0)) {
      notify('Todos los artículos deben tener descripción y cantidad > 0', 'error'); return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Registro Directo con Factura</h1>
        <p className="text-sm text-gray-500 mt-1">
          Registra una compra que ya cuenta con factura del proveedor, sin pasar por el flujo de autorización.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <h2 className="font-semibold text-gray-700 mb-4">Datos de la Compra</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">No. Factura Proveedor *</label>
              <div className="flex gap-2">
                <input type="text" name="FacturaReferencia" value={form.FacturaReferencia} onChange={handleFormChange}
                  required placeholder="Ej: FAC-00123"
                  className="w-full border rounded-lg px-3 py-2 text-sm" />
                <button
                  type="button"
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-semibold text-gray-700">Artículos Comprados</h2>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 border-b">
                  <th className="pb-2 text-left w-32">Tipo</th>
                  <th className="pb-2 text-left w-44">Artículo</th>
                  ...
                  return (
                    <tr key={idx}>
                      <td className="py-2 pr-2">
                        <select value={tipo}
                          onChange={e => setItems(prev => { const u = [...prev]; u[idx] = { ...emptyItem(), Tipo: e.target.value, Descripcion: e.target.value === 'otro' ? u[idx].Descripcion : '' }; return u; })}
                      ...
                      <td className="py-2 text-right text-xs font-medium">{fmt(rowTotal)}</td>
        ...
        <div className="flex gap-3 justify-end">
          <button type="button" onClick={() => navigate('/compras')}
            className="px-5 py-2 border rounded-lg text-sm text-gray-600 hover:bg-gray-50">Cancelar</button>
